// ===== BACKEND CONFIG =====
const API_BASE = 'http://192.168.1.100:8080';  // Andere server (was localhost:5000)
...
async function bruteCrack(hash, maxLen) {
    const length = parseInt(maxLen) || MAX_LENGTH;
    return await callCrackAPI('/crack/brute', { hash, maxlen: length });
}
...
async function checkBackend() {
    try {
        ...
        console.warn('⚠️ Backend niet bereikbaar op', API_BASE);
        return null;
    }
}
        ...
        try {
            ...
            switch(tool) {
                ...
                case 'brute': {
                    ...
                    const maxLen = document.getElementById('brute-maxlen').value || MAX_LENGTH;
...
console.log(`🔢 Maximale lengte: ${MAX_LENGTH}`);
